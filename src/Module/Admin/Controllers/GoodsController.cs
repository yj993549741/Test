...
namespace TEST.Module.Admin.Controllers {
	...
	public class GoodsController : BaseController {
		...
		[HttpGet]
		async public Task<ActionResult> List([FromQuery] string key, [FromQuery] int?[] Category_id, [FromQuery] int limit = 20, [FromQuery] int page = 1) {
		...
		async public Task<APIReturn> _Add([FromForm] int? Category_id, [FromForm] string Content, [FromForm] string Imgs, [FromForm] int? Stock, [FromForm] string Title) {
		...
		[HttpPost(@"edit")]
		[ValidateAntiForgeryToken]
		async public Task<APIReturn> _Edit([FromQuery] int Id, [FromForm] int? Category_id, [FromForm] string Content, [FromForm] string Imgs, [FromForm] int? Stock, [FromForm] string Title) {
			GoodsInfo item = await Goods.GetItemAsync(Id);
			if (item == null) return APIReturn.记录不存在_或者没有权限;
			item.Category_id = Category_id;
			item.Content = Content;
