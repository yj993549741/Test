	...
	public partial class Goods : IDAL {
		...
		public partial class SqlUpdateBuild {
			...
			public override string ToString() {
				...
				if (string.IsNullOrEmpty(_where)) throw new Exception("防止 TEST.DAL.Goods.SqlUpdateBuild 误修改，请必须设置 where 条件。");
			...
			public SqlUpdateBuild Set(string field, string value, params SqlParameter[] parms) {
				if (value.IndexOf('\'') != -1) throw new Exception("TEST.DAL.Goods.SqlUpdateBuild 可能存在注入漏洞，不允许传递 ' 给参数 value，若使用正常字符串，请使用参数化传递。");
