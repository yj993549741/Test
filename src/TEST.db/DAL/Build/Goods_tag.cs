...
namespace TEST.DAL {
	...
	public partial class Goods_tag : IDAL {
		...
		internal class TSQL {
			...
			internal static readonly string Sort = "a.[goods_id]";
		...
		public object GetItem(SqlDataReader dr, ref int dataIndex) {
			...
			if (!dr.IsDBNull(++dataIndex)) item.Tag_id = dr.GetInt32(dataIndex);
			return item;
		}
		...
		public Goods_tagInfo Delete(int? Goods_id) {
			Goods_tagInfo item = null;
			SqlHelper.ExecuteReader(dr => { item = BLL.Goods_tag.dal.GetItem(dr); }, string.Concat(TSQL.Delete, @"[goods_id] = @goods_id"),
		...
		public partial class SqlUpdateBuild {
			...
			public SqlUpdateBuild(List<Goods_tagInfo> dataSource, bool isRefershDataSource) {
				...
				_itemsDic = _dataSource == null ? null : _dataSource.ToDictionary(a => $"{a.Goods_id}");
			...
			public override string ToString() {
				...
				if (string.IsNullOrEmpty(_where)) throw new Exception("防止 TEST.DAL.Goods_tag.SqlUpdateBuild 误修改，请必须设置 where 条件。");
			...
			public int ExecuteNonQuery() {
				...
				foreach (var newitem in newitems) {
					if (_itemsDic.TryGetValue($"{newitem.Goods_id}", out var olditem)) foreach (var a in _setAs.Values) a(olditem, newitem);
					else {
						_dataSource.Add(newitem);
						_itemsDic.Add($"{newitem.Goods_id}", newitem);
					}
				}
			...
			public SqlUpdateBuild Set(string field, string value, params SqlParameter[] parms) {
				if (value.IndexOf('\'') != -1) throw new Exception("TEST.DAL.Goods_tag.SqlUpdateBuild 可能存在注入漏洞，不允许传递 ' 给参数 value，若使用正常字符串，请使用参数化传递。");
			...
			public SqlUpdateBuild SetGoods_idIncrement(int? value) {
				if (_dataSource != null && _setAs.ContainsKey("Goods_id") == false) _setAs.Add("Goods_id", (olditem, newitem) => olditem.Goods_id = newitem.Goods_id);
				return this.Set("[goods_id]", string.Concat("[goods_id] + @goods_id_", _parameters.Count), 
					new SqlParameter { ParameterName = string.Concat("@goods_id_", _parameters.Count), SqlDbType = SqlDbType.Int, Size = 4, Value = value });
			}
		...
		async public Task<(object result, int dataIndex)> GetItemAsync(SqlDataReader dr, int dataIndex) {
			...
			if (!await dr.IsDBNullAsync(++dataIndex)) item.Tag_id = await dr.GetFieldValueAsync<int>(dataIndex);
			return (item, dataIndex);
		}
		async public Task<Goods_tagInfo> DeleteAsync(int? Goods_id) {
			Goods_tagInfo item = null;
			await SqlHelper.ExecuteReaderAsync(async dr => { item = await BLL.Goods_tag.dal.GetItemAsync(dr); }, string.Concat(TSQL.Delete, @"[goods_id] = @goods_id"),
				new SqlParameter { ParameterName = "@goods_id", SqlDbType = SqlDbType.Int, Size = 4, Value = Goods_id });
			return item;
		}
