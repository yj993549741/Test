...
namespace TEST.BLL {
	...
	public partial class Tag {
		...
		internal static readonly TEST.DAL.Tag dal = new TEST.DAL.Tag();
		...
		public static TEST.DAL.Tag.SqlUpdateBuild UpdateDiy(int Id) => new TEST.DAL.Tag.SqlUpdateBuild(new List<TagInfo> { new TagInfo { Id = Id } }, false);
		public static TEST.DAL.Tag.SqlUpdateBuild UpdateDiy(List<TagInfo> dataSource) => new TEST.DAL.Tag.SqlUpdateBuild(dataSource, true);
		public static TEST.DAL.Tag.SqlUpdateBuild UpdateDiyDangerous => new TEST.DAL.Tag.SqlUpdateBuild();
		...
		internal static void RemoveCache(IEnumerable<TagInfo> items) {
			...
			foreach (var item in items) {
				keys[keysIdx++] = string.Concat("TEST_BLL:Tag:", item.Id);
			}
		...
		public static TagInfo GetItem(int Id) => SqlHelper.CacheShell(string.Concat("TEST_BLL:Tag:", Id), itemCacheTimeout, () => Select.WhereId(Id).ToOne());
		...
		async public static Task<TagInfo> GetItemAsync(int Id) => await SqlHelper.CacheShellAsync(string.Concat("TEST_BLL:Tag:", Id), itemCacheTimeout, () => Select.WhereId(Id).ToOneAsync());
