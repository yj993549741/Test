...
namespace es.DAL {
	...
	public partial class Goods : IDAL {
		...
		internal class TSQL {
			...
			internal static readonly string InsertField = "[id], [category_id], [content], [create_time], [imgs], [stock], [title], [update_time]";
			internal static readonly string InsertValues = "@id, @category_id, @content, @create_time, @imgs, @stock, @title, @update_time";
		...
		public (string sql, SqlParameter[] parms) InsertMakeParam(IEnumerable<GoodsInfo> items) {
			...
			var values = "";
			var parms = new SqlParameter[itemsArr.Length * 8];
