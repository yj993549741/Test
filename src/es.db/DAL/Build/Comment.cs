...
namespace es.DAL {
	...
	public partial class Comment : IDAL {
		...
		internal class TSQL {
			...
			internal static readonly string InsertField = "[id], [goods_id], [content], [create_time], [nickname], [update_time]";
			internal static readonly string InsertValues = "@id, @goods_id, @content, @create_time, @nickname, @update_time";
		...
		public (string sql, SqlParameter[] parms) InsertMakeParam(IEnumerable<CommentInfo> items) {
			...
			var values = "";
			var parms = new SqlParameter[itemsArr.Length * 6];
